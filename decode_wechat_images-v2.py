...
def cmd_setting_wx_path():
	...
	tmpPath = input("\n请输入微信文件所在目录:	")
	if tmpPath=="":
		tmpPath = os.getenv("WX_DATA")
		print("WX_DATA=%s" % tmpPath)
...
def cmd_gen_sub_year_month(sSubKey):
	nYear = int(datetime.datetime.now().strftime("%Y"))
	nMonth = int(datetime.datetime.now().strftime("%m"))
...
# 定义统计指定文目录大小的函数
# 可使用 os.stat().st_size	替换 os.path.getsize()
def	dir_size_by_create_datetime(dir,dtFlag):
	strDtFlag = dtFlag.strftime("%Y-%m-%d") + " 00:00:00"
	size = 0
	# 指定目录是否为文件
	if os.path.isfile(dir):
		size +=	os.path.getsize(dir)
	...
	if os.path.isdir(dir):
		...
		for	i in dir_lst:
			...
			if os.path.isfile(file):
				...
				tm = time.localtime(t)
				strTm = time.strftime('%Y-%m-%d %H:%M:%S', tm)
				#print(strTm)
				#print(strDtFlag)
				#print(type(t))
				#print(type(tm))
				#print(type(times))
				#print(type(datetime.datetime(times)))
				#print(type(dtFlag.strftime("%Y-%m-%d %H:%M:%S")))
				#print(dtFlag.strftime("%Y-%m-%d %H:%M:%S"))
				#print(relativedelta(tm, dtFlag))
				if strTm > strDtFlag:
					size +=	os.path.getsize(file)
			if os.path.isdir(file):
				size +=	dir_size_by_create_datetime(file, dtFlag)
	return size

#统计最近3天的增量值
def cmd_sum_diskspace_of_last_3day(nDay):

	#t = os.path.getmtime('b.json')
	#timeStruce = time.localtime(t)
	#times = time.strftime('%Y-%m-%d %H:%M:%S', timeStruce)
	#print(times)
	print("当前时间往前推3天：",datetime.datetime.now()+ relativedelta(days=-3))
	tPrev = datetime.datetime.now()+ relativedelta(days=-3)
	print(tPrev)
	...
	for aPath in g_wx_sub_paths:
		...
		if aPath.find("File")==0:
			if sFlag != "File":
				...
				nSubSize =0
				sFlag = "File"
		nSize = dir_size_by_create_datetime(os.path.join(g_wx_files_path, aPath), tPrev)
		nTotalSize += nSize
		nSubSize += nSize
		if nSize>0:
...
def menu_cmd():
	...
	while True:
		...
		print("	3、检查Image/Video/File 磁盘最近3天的增量\n")
		print("	4、解码特定月份的Image文件\n")
		print("	0、退出程序(q)\n")
		print("==========================================\n")
		...
		if flag	== 0:
			...
			cmd_sum_diskspace_of_last_3day(3)
		elif flag == 4:
			cmd_decode_sub_path()
		else:
			print("请输入正确的操作编号！！")
...
if __name__	== '__main__':
	...
	#print(datetime.now())
	#print("当前时间往前推3天：", datetime.now() + relativedelta(days=-3))
	#tPrev = datetime.now() + relativedelta(days=-3)
	#print(tPrev)
	menu_cmd()
